...
export default function MediaPanel() {
  ...
  const {
    mediaAssets, importProgress, project, tracks,
    addMediaAsset, removeMediaAsset, setImportProgress,
    addTrack, addClip,
  } = useEditorStore();
  ...
  const processFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      let trackId: string;
      const existingTrack = tracks.find((t) => t.type === (mediaType === 'image' ? 'video' : mediaType));
      if (existingTrack) {
        trackId = existingTrack.id;
      } else {
        trackId = addTrack(mediaType === 'image' ? 'video' : mediaType);
      }
      ...
      const clipDuration = duration || 5; // default 5s for images

      const newClip: Omit<TimelineClip, 'id'> = {
        trackId,
        mediaId: id,
        type: mediaType === 'image' ? 'video' : mediaType,
  ...
  }, [addMediaAsset, addTrack, addClip, setImportProgress, tracks, dimensions]);
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <PanelSection title={`Assets (${assets.length})`} defaultOpen={true}>
        {assets.length === 0 ? (
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {assets.map((asset) => (
              <motion.div
                ...
                className="group relative rounded-lg overflow-hidden bg-zinc-900 border border-zinc-800 hover:border-zinc-600 transition-colors cursor-pointer"
              >
                {/* Thumbnail */}
