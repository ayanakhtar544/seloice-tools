...
export default function VideoCompressorClient() {
  ...
  const handleCompress = async () => {
    ...
    if (!ffmpeg) {
      alert("Compression engine failed to load. Please refresh and try again.");
      return;
    }
    ...
    <div className="w-full  bg-[#050505] text-white font-sans selection:bg-orange-500/30 pb-20">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 pt-8 md:pt-16">
        ...
        <div className="text-center mb-12">
          ...
          <h2 className="text-4xl md:text-6xl font-black italic tracking-tighter mb-4">VIDEO <span className="text-orange-400">COMPRESSOR</span></h2>
        ...
        <div className="bg-[#111] border border-white/10 rounded-[2rem] p-6 md:p-10 shadow-2xl relative overflow-hidden">
          ...
          <div 
            onClick={() => !isProcessing && !isLoadingEngine && fileInputRef.current?.click()}
            ...
            className={`border-2 border-dashed rounded-3xl p-10 flex flex-col items-center justify-center text-center transition-all duration-300
              ${isProcessing || isLoadingEngine ? 'border-orange-500/20 bg-orange-500/5 cursor-wait' : 
                'border-white/10 hover:border-orange-500/50 hover:bg-white/[0.02] cursor-pointer'}`}
          >
            <input type="file" accept="video/*" className="hidden" ref={fileInputRef} onChange={handleFileChange} disabled={isProcessing || isLoadingEngine} aria-label="Upload video file" />
            ...
            {isProcessing ? (
              ...
              <div className="flex flex-col items-center">
                ...
                <h3 className="text-xl font-black mb-2 italic text-gray-300">
                  {isLoadingEngine ? "LOADING ENGINE..." : "CLICK TO UPLOAD VIDEO"}
                </h3>
                <p className="text-sm text-gray-500 font-medium max-w-xs mx-auto">
                  {isLoadingEngine ? "Downloading WASM compression core." : "Supports MP4, MOV, WEBM and more."}
                </p>
              </div>
            )}
          </div>
