...
import { LayoutDashboard, FileText, PlusCircle, Settings, LogOut, Home } from 'lucide-react';
...
export default function AdminSidebar() {
  ...
  return (
    <div className="w-64 h-screen bg-[#0a0a0f] border-r border-white/5 flex flex-col p-6 sticky top-0 shrink-0">
      ...
      <nav className="flex-1 space-y-2">
        {MENU_ITEMS.map((item) => (
          <Link 
            key={item.href} 
            href={item.href}
            className={`flex items-center gap-4 px-4 py-3 rounded-xl font-bold text-sm transition-all ${pathname === item.href ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'text-gray-500 hover:bg-white/5 hover:text-white'}`}
          >
            {item.icon}
            {item.name}
          </Link>
      ...
      <div className="pt-6 border-t border-white/5 flex flex-col gap-2">
        <Link href="/" className="flex items-center gap-4 px-4 py-3 text-gray-500 hover:text-emerald-400 transition-all font-bold text-sm">
